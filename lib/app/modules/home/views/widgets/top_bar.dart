    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const Text('DeClient', style: TextStyle(color: AppColors.foreground, fontSize: 17, fontWeight: FontWeight.w700)),
          const SizedBox(width: 8),
          const Text('Postman sederhana', style: TextStyle(color: AppColors.comment, fontSize: 12)),
          const Spacer(),
          _EnvSelector(),
          const SizedBox(width: 8),
          IconButton(
...
class _EnvSelector extends GetView<EnvironmentController> {
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      final items = controller.environments;
      final active = controller.activeId.value;
      return Container(
        ...
        child: DropdownButtonHideUnderline(
          child: DropdownButton<String?>(
            value: active,
            hint: const Text('No Environment', style: TextStyle(color: AppColors.comment, fontSize: 13)),
            ...
            items: [
              ...
              ...items.map((e) => DropdownMenuItem<String?>(value: e.id, child: Text(e.name))),
            ],
...
class _ImportExportMenu extends GetView<CollectionController> {
  ...
  Widget build(BuildContext context) {
    return PopupMenuButton<String>(
      ...
      icon: const Icon(LucideIcons.arrowLeftRight, size: 19),
      onSelected: (v) {
        if (v == 'export') {
          showExportDialog();
        } else if (v == 'postman') {
          showPostmanImportDialog();
        }
      },
      itemBuilder: (_) => const [
        PopupMenuItem(value: 'export', child: Text('Export (JSON)')),
        PopupMenuItem(value: 'postman', child: Text('Import Postman v2.1')),
      ],
    );
  }
}
